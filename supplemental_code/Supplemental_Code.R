...
outlier_mean_abs_trimethyl_dat <- mean_abs_trimethyl_dat %>%
  ...
  mutate(outlier = value > median(value, na.rm = TRUE) + IQR(value, na.rm = TRUE) * 2) %>%
  ungroup
...
outlier_mean_rel_trimethyl_dat <- mean_rel_trimethyl_dat %>%
  group_by(assay, factor(timepoint * 60)) %>%
  mutate(outlier = value > median(value, na.rm = TRUE) + IQR(value, na.rm = TRUE) * 2 | value < median(value, na.rm = TRUE) - IQR(value, na.rm = TRUE) * 2) %>%
  ungroup
...
fig4d_hist <- ggplot(data = beta_glmm_dat %>%
                       filter(gene %in% intersect(high_confidence_genes_down, high_confidence_genes_up),
                              assay %in% c("writer_add", "writer_loss")),
...
## ZIO beta GLMM rates by RNA abundance (Figure 4E)
...
# Writer loss
writer_loss <- ggplot(data = mean_rel_trimethyl_dat %>% 
                        filter(assay == "writer_loss"), 
                      aes(x = timepoint, y = value, color = gene)) + 
  geom_point() + 
  geom_line() + 
  scale_color_grey() + 
...
##
##############################################
single_gene_dat <- rel_trimethyl_dat %>%
  filter(gene %in% rel_trimethyl_dat$gene[1]) %>%
